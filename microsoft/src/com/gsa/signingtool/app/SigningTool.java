...
public class SigningTool {
    ...
    protected SigningTool(File file, char[] pin) {
        ...
        try {
            ...
            logger.info("Selecting the first card reader slot "
                    + "found in ArrayList");
            if (readers.isEmpty()) {
                logger.severe("Issue selecting card reader and verifying "
                        + "card is inserted.");
                Gui.status.append(dateFormat.format(new Date()) 
                        + " - Issue detecting card reader and/or if card "
                        + "is inserted.\n");
                Gui.errors = true;
            ...
            String configName = "name=" 
                    ...
                    + "slot=" 
                    + slotID;
            logger.info("Output of information needed to connect SunPKCS11"
                    + " Provider.\n" 
                    + configName);
            ...
            try {
                ...
                logger.info("Trying to connect to smart card using SunPKCS11 "
                        + "and user PIN");
                piv.load(null, pin);
            ...
            Enumeration aliasesEnum = piv.aliases();
                while (aliasesEnum.hasMoreElements()) {
                   ...
                   if (keyUsage[0] && keyUsage[1]) {
                       logger.info("Found a signing certificate with Digital"
            ...
            if (signingCerts.size()==1) {  
                ...
                try {
                    logger.info("Checking the expiration date of the "
                            + "signing certificate");
                ...
                if (Gui.revocationCheckBox.getState()) {
                    try {
                        logger.info("Checking Revocation Status of "
                                + "signing certificate...");
                        ...
                        if (revokeStatus == "REVOKED") {
                            Gui.status.append(dateFormat.format(new Date()) 
                                    + " - Unable to sign. Signing Certificate "
                                    + "is REVOKED");
                        }
                    } catch (Exception e) {
                        Gui.status.append(dateFormat.format(new Date()) 
                ...
                logger.info("Trying to create a ContentSigner using SHA256RSA "
                        + "algorithm, SunPKCS11 Provider, and Signing Cert "
                        + "Private Key...");
                ContentSigner sha2 = new JcaContentSignerBuilder(
                        "SHA256withRSA").setProvider("SunPKCS11-opensc").build(privateKey);
                ...
                String tooManySigners = "Unable to sign. More than 1 "
                ...
                Gui.errors = true;
                errorDesc = "Unable to sign. No signing certificates "
                        + "found on the card.";
                Gui.status.append(dateFormat.format(new Date()) 
                        + " - " 
                        + errorDesc 
