...
public class CheckRevocationStatus {
    public static X509Certificate fetchIssuerCert(X509Certificate cert) {
        ...
        try {
            Gui.logger.info("Trying to create new file to store issuer p7c file");
        ...
        try {
            Gui.logger.info("Trying to get AiA from end-entity certificate");
            byte[] extensionValue = 
                    ((X509Certificate)cert).getExtensionValue("1.3.6.1.5.5.7.1.1");
            DEROctetString string = 
                    (DEROctetString) 
                    (new ASN1InputStream(new ByteArrayInputStream(extensionValue)).readObject()); 
            ASN1Sequence seq = 
                    ASN1Sequence.getInstance(
                    ASN1Primitive.fromByteArray(DEROctetString.getInstance(string).getOctets()));
            access = 
                    AuthorityInformationAccess.getInstance(seq);
        ...
        try {
            ...
            Gui.logger.info("Looping through the accessDescriptions to get the different end-points");
                ...
                if(gn.getTagNo() == GeneralName.uniformResourceIdentifier) {
                    ...
                    if (serviceAddr.toLowerCase().startsWith("http") 
                        ...
                        try {          
                            ...
                            Gui.logger.info("Downloading the issuer cert p7c file");
                            org.apache.commons.io.FileUtils.copyURLToFile(url, issuerChain);
                            Gui.logger.info("Saving the issuer cert p7c file");
                            FileInputStream fis = new FileInputStream(issuerChain);
                            Gui.logger.info("File location: " 
                                    + issuerChain.getAbsolutePath());
                            Gui.logger.info("Generating a new certificate to store issuer cert ");
                            CertificateFactory cf = CertificateFactory.getInstance("x.509");
                            Iterator iter = cf.generateCertificates(fis).iterator();
                            Gui.logger.info("Iterating through the p7c file");
                            while (iter.hasNext()) {
                                issuerCert = (X509Certificate)iter.next();
                                issuingCerts.add(issuerCert);
                                Gui.logger.info("Seeing if their is a match between "
                                + "the end-entity issuerDN and certificates in p7c file subjectDN");
                                if (issuerCert.getSubjectDN().equals(cert.getIssuerDN())) {
                                    issuerMatch = true;
                                    Gui.logger.info("Found a match! The issuer of" 
                                            + " the end-entity cert's serial number is: " 
                                            + issuerCert.getSerialNumber());
                                    exactIssuerCert = issuerCert;
                                }
                            }
                            if (issuerMatch = false) {
                                Gui.logger.severe("Unable to check revocation "
                                        + "status. Issue finding issuer cert");
                                return null;
                            }
                        } catch (Exception e) {
                            Gui.logger.log(Level.FINEST, e.getMessage(), e);
    ...
    public static String checkRevocation(X509Certificate end, X509Certificate issuer) {
        ...
        try {
            ...
            Gui.logger.info("Revocation Status is: " 
                    + revocationStatus);
        } catch (Exception ex) {
            if (SigningTool.expired == true) {
                Gui.status.append(dateFormat.format(new Date()) 
                        + " - Unable to sign. Revocation Status returned an error.\n" 
                        ...
                        + "\n");
                System.out.println("Unable to sign. The signing certificate is "
                        + "expired. Also, unable revocation status returned an error.");
                Gui.logger.log(Level.FINEST, ex.getMessage(), ex);
                return null;
            } else {
                Gui.status.append(dateFormat.format(new Date()) 
                        + " - Unable to get revocation status.\n ");
                System.out.println("Unable to sign. The revocation check for " 
                        + "your signing cert failed. If you believe this is in "
                        + "err, disable revocation checking in the menu.");
                Gui.logger.log(Level.FINEST, ex.getMessage(), ex);
                return null;
            }
